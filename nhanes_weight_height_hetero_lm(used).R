...
for(i in seq(perms)){
    ...
    dat <- nhanes[indicies, ]
    dat_train <- dat[1:1000, ] #500 for mean model
    dat_hetero <- dat[1001:2000, ] #500 for heteroskedastic 
    ...
    sigma_test <- abs(hetero_test - dat_hetero$Height)
    dat_hetero <- cbind(dat_hetero, sigma_test)
    
    mod_sig <- lm(sigma_test ~ Weight, data = dat_hetero)
    sig_cal <- predict(mod_sig, newdata = dat_cal)
    sig_out <- predict(mod_sig, newdata = dat_out)
    
    pred_cal <- predict(mod_train, newdata = dat_cal)
    
    upper_scores <- score_fun(pred_cal, dat_cal$Height, sig_cal)
    ...
    param_dat <- rbind(dat_train, dat_hetero[, 1:3], dat_cal)
